...
int main(int argc, char **argv)
{
    ...
    free(coo_row);
    //didnt malloc these because just made them point to the ones above
    //free(coo_col);
    //free(coo_val);
    free(res_coo);
...
void SpMV_CSR(uint32_t row, int *row_ptr, int *col_ind, int *val, int *vec, 
              int *res)
{
    //for each row and each non zero multiply the vector to the matrix
    for(int i = 0; i < row; i++)
    {
    	for(int j = row_ptr[i]; j < row_ptr[i+1]; j++)
    	{
    		//stores the final result in the the res array
    		res[i] = res[i] + (val[j] * vec[col_ind[j]]);
    	}
    }
...
void convert_csr_to_coo(uint32_t row, int *row_ptr, int *col_ind, int *val, 
                        int **coo_row, int **coo_col, int **coo_val)
{
	//set col and val equal the csr counterpart since its the same
	//*coo_val = (int*)malloc(sizeof(int) * nnz);
	*coo_col = col_ind;
	*coo_val = val;

	//create a temp row by using the last index of rowptr
	int *coo_row_ = NULL;
	coo_row_ = (int*) malloc(sizeof(int) * row_ptr[row]);

	for(int i = 0; i < row; i++)
	{
		//sets the row equal to i and increases i at the beginning of each new row
		//through row pointer
		for(int j = row_ptr[i]; j < row_ptr[i+1]; j++)
		{
			coo_row_[j] = i;
	...
	//set the new row to point at the temp one;
	*coo_row = coo_row_;
              ...
              int *coo_val, int *vec, int *res)
{
	...
	//similar to csr but use res at coo_row pointer of index i

	for(int i = 0; i < nnz; i++)
	{
		temp = coo_val[i] * vec[coo_col[i]];
		res[coo_row[i]] = res[coo_row[i]] + temp;
	}    
}
