#include "matrix.h"
#include <math.h> //to use infinity (could have used a large number but thought this was better)
...
void init_2d_array(int ***arr, int rows, int cols)
{
    ...
    int **tmp_array = (int**) malloc(sizeof(int*) * rows);
    int i, j;
    for(i = 0; i < rows; i++) {
        tmp_array[i] = (int*) malloc(sizeof(int) * cols);
    }
    //fill temp array with zeros
    for(i = 0; i < rows; i++)
    {
        for(j = 0; j < cols; j++)
        {
