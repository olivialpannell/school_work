...
//     char *tmp_file2 = (char *)"temporary2.pbm";
//     black_white_filter(filename_2, tmp_file2);

//     int bw_height = get_PBM_height(tmp_file2);
//     int bw_width = get_PBM_width(tmp_file2);

//     pixel **b_and_w = read_exported_PBM(tmp_file2);   // Black and white pixel array of image
    
//     remove_file(tmp_file2);

//     char *tmp_file = (char *)"temporary.pbm"; // Creates temporary file variable 
//     create_blank_image(tmp_file, bw_width, bw_height);


//     pixel **tmp_img = read_exported_PBM(tmp_file); // Creates a blank image
//     remove_file(tmp_file);

//     for(int i = 0; i < bw_height; i++)
//     {
//         for (int j = 0; j < bw_width; j++)
//         {
//             if(b_and_w[i][j].r == 0 && b_and_w[i][j].g == 0 && b_and_w[i][j].b == 0)
//             {
//                 tmp_img[i][j].r = arr[i][j].r;
//                 tmp_img[i][j].g = arr[i][j].g;
//                 tmp_img[i][j].b = arr[i][j].b;
//             }
//             else
//             {
//                 tmp_img[i][j].r = 255;
...
//     f.open(name);   // Opens and names the file with the specified 'name'.
//     f << "P3\n";    // Writes magic number to file (tells the computer its an r/g/b image).
//     f << bw_width << " " << bw_height << "\n"; // Writes width and height to the file.
//     f << "255\n";   // Writes the color range you're working with to the file (standard = 255).
//     for(int i = 0; i < bw_height; i++)     // Iterates through height of the image.
//     {
//         for (int j = 0; j < bw_width; j++) // Iterates through width of the image.
//         {
//             if(j == bw_width-1){
//             // Writes r/g/b pixel data so there isn't an extra space at the end of each row.
//                 f << tmp_img[i][j].r << " " << tmp_img[i][j].g << " " << tmp_img[i][j].b;
//             }
...
void blur_filter(char *filename, char *name){
    ...
    */
    ...
    {
        int average_r; //Variable to track the average of the neighboring r pixels
        int average_g; //Variable to track the average of the neighboring g pixels 
        int average_b; //Variable to track the average of the neighboring g pixels  
        
        // ------> Current logic 
        for(int i = 2; i < height-3; i++){
            for(int j = 2; j < width-3; j++){
                
                average_r = (arr[i-2][j-2].r + arr[i-2][j-1].r + arr[i-2][j].r + arr[i-2][j+1].r + arr[i-2][j+2].r +
                ...
                average_g = (arr[i-2][j-2].g + arr[i-2][j-1].g + arr[i-2][j].g + arr[i-2][j+1].g + arr[i-2][j+2].g +
                ...
                average_b = (arr[i-2][j-2].b + arr[i-2][j-1].b + arr[i-2][j].b + arr[i-2][j+1].b + arr[i-2][j+2].b +
...
void popart_filter(char *filename, char *name){
    ...
    */
    ...
    free_matrix(height, arr);
    free_matrix(height, red);
    free_matrix(height, green);
    free_matrix(height, blue);
    free_matrix(height, yellow);
    free_matrix((height * 2), temp);
}
