...
void mirror_h_filter(char *filename, char *name){
    ...
    */
    ...
    for(int i = 0; i < height; i++)
    {
        for(int j = 0; j < width; j++)
        {
            arr[i][width - 1 - j].r = arr[i][j].r;
            arr[i][width - 1 - j].g = arr[i][j].g;
            arr[i][width - 1- j].b = arr[i][j].b;
...
void kaleidoscope(char *filename, char *name){
    ...
    for(int i = 0; i < height; i++){
            ...
            if((0 <= i_copy_2 || i_copy_2 <= height) && ( 0 <= j_copy_2 || j_copy_2 <= width))
            {
    ...
    free_matrix(h1, arr_even);
    free_matrix(h2, arr_crop);
    free_matrix(h2, arr_sq);
    free_matrix(h2, arr_tri);
    free_matrix(h2, arr_tri_rot);
    free_matrix(h2, arr_tri_rot_t);
    free_matrix(h2, arr_tri_rot_s);
    free_matrix(hig_sq, fin_arr);
    free_matrix(hig_sq, mirr);
    free_matrix(double_height, final);
}
